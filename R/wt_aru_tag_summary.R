  ...
  # Compose SQL query
    query <- glue_sql(
      "SELECT
            ...
            visibility_type as visibility,
            project_year as year,
            data_set_nm || '-' || site_name || '-' || station_name as station_key,
            site_public_latitude,
            site_public_longitude,
            station_latitude,
            station_longitude,
            station_buffer_size,
            recording_file_name,
            recording_file_type,
            recording_audio_length,
            to_char(recording_date,'YYYY-MM-DD') as recording_date,
            to_char(recording_date, 'HH24:mi:ss') as recording_time,
            to_char(recording_date,'J') as JD,
            recording_spectrogram_profile,
            recording_source_file_path,
            recording_channels,
            task_id,
            task_mean_decibels,
            task_max_decibels,
      ...
      FROM aru.tag
            ...
            JOIN common.lu_visibility ON project_visibility_id = visibility_id
